...
(define-object time-ratio-1-four-bar-linkages (base-object)
  :input-slots
  (
   (rocker-center)
   (rocker-length)
  ...
  (
   ...
   ;; crank x y
   (crank-x (- (the dis-crank-rocker) (* (cos (degrees-to-radians (the angle-crank-frame))) (the crank-length))))
   (crank-y (* (sin (degrees-to-radians (the angle-crank-frame))) (the crank-length)))
   ;; rocker x y
   (rocker-x (+ (get-x (the rocker-point center)) 
		(* (sin (degrees-to-radians  (- (the rocker-angle) (the angle-crank-frame)))) 
		   (the rocker-length) -1)))
